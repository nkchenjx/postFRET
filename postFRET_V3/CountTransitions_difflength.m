...
FRETTraj = Data(:,4);
TrajL = length(FRETTraj);
bin_t = Data(:,1);
states = FRETStates;
numStates =  size(FRETStates,2);
tempstate = zeros(numStates,TrajL);
...
% end
numMol = length(Datalength);
eachTrajLength2 = Datalength;
h(eachTrajLength2,:) = 0;
...
transTime = numtransitionfit.*lifetime; %a matrix of total time of a state transfer to another
transTime(isnan(transTime))=0; %if no transfer the time is set to 0
StateTime = sum(transTime,1); %the total observed time for each state

for i = 1:numStates
    %rateconstantsfit(i,:) = numtransitionfit(i,:)./numtransition(numStates+1,:);  %rate constants of each transition
    rateconstantsfit(i,:) = numtransitionfit(i,:)./StateTime; % rate constants of each transition
end

 rateconstantsfit(isnan(rateconstantsfit)) = 0; %if there is no transition found, set the rate constant to 0
