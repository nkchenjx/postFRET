...
totalsignal = config.sumIaa*config.delta_t/config.bintime;
...
for x = 1:numState % initialize transProb
    ...
    stayP(x) = exp(-sumRate(x)*delta_t); % this probability to stay
    transP(x) =  1-stayP(x);  % this probability to transfer
    for y = 1:numState
        if x == y
            transProb(y, x) = stayP(x);
        else    
            transProb(y, x) = rate(y,x)*transP(x)/sumRate(x);
        end
    end
    for y = 2:numState
        transProb(y, x) = transProb(y-1, x) + transProb(y, x);
    end
end


stateLifetime = 1./sumRate;
...
rng('shuffle');
molecules(numMol) = struct('traject', []);
...
%    molecules(m).traject = ones(1,cycle(m)); %initialize all molecules in the whole time in state 1
    ...
    for f = numState:-1:1
       if P <= sum(stateConcerntation(1:f)) 
          molecules(m).traject = ones(cycle(m), 1);
          molecules(m).traject(1) = f;
       end
    end
...
%             % consider pregenerate for every states.
            newmolecules = numState;
            for f = numState:-1:1  % final state
                 if p1 <= transProb(f, istate)
...
for i = 1:trajL
        ...
        t = totalsignal;  % donor + acceptor total counts
