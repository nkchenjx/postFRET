function [rateSimu] = findRateSimuNoise(config, rate)
...
%% add noise to data before binning
% noise = zeros(trajL,1);
...
% bknoise = ones(trajL,1).*config.backgroundnoise;
...
DataBin(:,2) = round(abs(DataBin(:,2)));
noise = zeros(trajL,1);
% noise = JCawgn(noise,ampsnr,'measured', 'linear')-noise; %create white noise
noise = JCawgn(noise,config.rawnoise);
DataBin(:,3) = DataBin(:,3) + noise; %acceptor
DataBin(:,3) = round(abs(DataBin(:,3)));
%disp('noise added to both doner channel and acceptor channel');
DataBin(:,4) = DataBin(:,3) ./ (DataBin(:,2) + DataBin(:,3));
